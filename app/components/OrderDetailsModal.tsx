      ...
      <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border border-gray-200 rounded-lg">
              ...
              {orderDetails.map((detail, index) => (
                <tr key={detail.id} className={`border-b border-gray-100 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                  <td className="p-3 text-gray-900 font-medium">{detail.productName}</td>
                  <td className="p-3 text-gray-900">{detail.quantity}</td>
        ...
        {orderDetails.length > 0 && (
          <div className="mt-4 text-sm text-gray-600">
            Tổng cộng: {orderDetails.length} sản phẩm
          </div>
        )}
      </div>
    </Modal>
