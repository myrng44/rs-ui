...
export default function Products() {
  ...
  const handleAutocompleteSearch = async (query: string): Promise<SearchResult[]> => {
	...
	const loadProducts = async (page: number = currentPage) => {
      ...
      const normalized: Product[] = response.elements.map((p) => ({
        id: p.id,
        sku: p.sku,
        name: p.name,
        description: (p as any).description ?? (p as any).desc ?? '', // support both 'description' and 'desc'
        unitPrice: p.unitPrice ?? 0,
        categoryId: (p.categoryId ?? 0) as number,
      }));
