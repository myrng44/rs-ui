		...
		<Layout>
			<div className='space-y-6'>
				<div className='flex justify-between items-center'>
					...
					<div className='text-sm text-gray-500'>Tổng {stocks.length} sản phẩm</div>
				</div>
									...
									stocks.map((stock) => {
										...
										return (
											<tr key={stock.id} className='border-b border-gray-100 hover:bg-gray-50'>
												<td className='p-4 font-medium text-gray-900'>{formatProductId(stock.productId)}</td>
				...
				{!loading && stocks.length > 0 && (
					<div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
						...
						<div className='bg-surface p-4 rounded-lg border border-gray-200'>
							<div className='text-2xl font-bold text-success'>{stocks.filter((s) => s.quantity > 50).length}</div>
						...
						<div className='bg-surface p-4 rounded-lg border border-gray-200'>
							<div className='text-2xl font-bold text-error'>{stocks.filter((s) => s.quantity === 0).length}</div>
